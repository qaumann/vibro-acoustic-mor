% Evaluate results and save them to csv, fig, and png
...
benchs = {'radiation', 'transmission', 'poroacoustic', ...
    'plate_48_rayleigh', 'plate_48_hysteretic', 'plate_48_rayleigh_single'};
...
%         methods = cellfun(@(x) erase(x, "single_"), methods, 'UniformOutput', false);
%     end
    ...
    %           (5) tf and rel error for max(r) per method
    ...
    %% (3) max rel error over order for all methods
    ...
    mor_methods = ["strint_avg", "strint_equi", "strint_linf", "minrel", "pod"];
    pre_methods = ["aaaa", "strprs"];
    proj_methods = ["tsimag", "tsreal", "osimaginput", "osrealinput", ...
        "osimagoutput", "osrealoutput"]; %, ..., "osinput", "osoutput", "ts"
    %     "fv", "pv", "p", "so", "vp", "v"];
    ...
    for m = proj_methods
        ...
        morscore_dat{idx} = [this_results.morscore_hinf]'; %#ok
        idx = idx+1;
    ...
    bar_names = {this_results.name};
    bar_names = strrep(bar_names, ['_' m{:}],'');
%     if strcmp(bench, 'plate_48_rayleigh')
%         bar_names = strrep(bar_names, 'single_','');
%     end
    bar_names = strrep(bar_names, '_', ' ');
    
    bar_cats = categorical(bar_names);
    bar_cats = reordercats(bar_cats, {'strint equi', 'strint avg', ...
        'strint avg aaaa', 'strint avg strprs', 'strint linf', ...
        'strint linf aaaa', 'strint linf strprs', 'minrel', ...
        'minrel aaaa', 'minrel strprs'});
    bar(bar_cats, cell2mat(morscore_dat))
    legend(proj_methods)
    
    if save_graphics_flag == true
        saveas(gcf, [dir_graphics '/' bench '_morscore.png'], 'png');
        saveas(gcf, [dir_graphics '/' bench '_morscore.fig'], 'fig');
    end
    close(1)
