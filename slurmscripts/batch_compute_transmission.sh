...
# BATCH_TRANSMISSION
# Queues compute scripts of the benchmark 'transmission' using all applicable methods.
...
for pm in 'tsimag' 'tsreal' 'osimaginput' 'osimagoutput' 'osrealinput' 'osrealoutput'; do
	for method in 'strint_avg' 'strint_linf' 'minrel'; do
		...
		sbatch -J tr_${method}_${pm}_strprs \
            --mail-user=${email} \
            --partition=long \
            --time=3-00:00:00 \
